...
(function () {
  ...
  function writeFreshSessionEnvelope(session, contextId) {
    if (session.status !== 'active' || !contextId) return;
    writeEnvelope({
  ...
  function recordSuccessfulResponse(pathname, method, init, payload) {
    ...
    if (method === 'POST' && pathname === '/api/game-sessions') {
      writeFreshSessionEnvelope(session, contextId);
      return;
    }
  ...
  function ensureResultStyle() {
    ...
    style.textContent =
      ...
      '.m4-compare-actions{display:flex;flex-wrap:wrap;align-items:center;gap:10px;margin:18px 0}' +
      '.m4-replay-copy{flex-basis:100%;margin:0;color:#596d75;font-size:.82rem}' +
      ...
      '.m4-comparison-replay{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-top:16px;padding-top:14px;border-top:1px solid #d5dcda}' +
      '.m4-comparison-replay p{flex-basis:100%;margin:0;color:#596d75;font-size:.82rem}' +
  ...
  async function restartFromResult(button) {
    const view = lastGameView;
    if (!view || !view.session || button.disabled) return;
    button.disabled = true;
    try {
      if (typeof window.request !== 'function') throw new Error('La partida todavía no está disponible.');
      const restarted = await window.request(
        '/api/game-sessions/' + encodeURIComponent(view.session.id) + '/restart',
        { method: 'POST', body: '{}' }
      );
      if (!restarted) throw new Error('No se pudo iniciar otra partida.');
      if (typeof window.focusCurrentSceneHeading === 'function') window.focusCurrentSceneHeading();
    } catch (error) {
      const notice = document.getElementById('notice');
      if (notice) notice.textContent = error instanceof Error ? error.message : 'No se pudo iniciar otra partida.';
      button.disabled = false;
    }
  }

  function makeReplayButton(id) {
    const button = document.createElement('button');
    button.id = id;
    button.className = 'primary';
    button.type = 'button';
    button.textContent = 'Jugar otra partida';
    button.addEventListener('click', function () { restartFromResult(button); });
    return button;
  }
  ...
  function ensureResultActions() {
    ...
    const sceneContent = document.querySelector('.scene[class*="result-"] .scene-content');
    if (!sceneContent) return;
    let actions = document.getElementById('m4-result-actions');
    if (!actions) {
